...
var preparedStatements = map[string]string{
	...
	"tcp_route_certificate_delete_by_route_id": tcpRouteCertificateDeleteByRouteIDQuery,
	"tcp_route_certificate_insert":             tcpRouteCertificateInsertQuery,
...
WHERE id = $1`
	tcpRouteListQuery = `
SELECT r.id, r.parent_ref, r.service, r.port, r.leader, r.drain_backends, r.domain, r.tls_mode, r.managed_certificate_domain, r.created_at, r.updated_at, c.id, c.cert, c.key, c.created_at, c.updated_at FROM tcp_routes as r
LEFT OUTER JOIN tcp_route_certificates AS rc on r.id = rc.tcp_route_id
LEFT OUTER JOIN certificates AS c ON c.id = rc.certificate_id
WHERE r.deleted_at IS NULL`
...
WHERE r.parent_ref = $1 AND r.deleted_at IS NULL`
	tcpRouteInsertQuery = `
INSERT INTO tcp_routes (parent_ref, service, port, leader, drain_backends, domain, tls_mode, managed_certificate_domain)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
...
WHERE r.id = $1 AND r.deleted_at IS NULL`
	tcpRouteUpdateQuery = `
UPDATE tcp_routes SET parent_ref = $1, service = $2, port = $3, leader = $4, domain = $5, tls_mode = $6, managed_certificate_domain = $7
WHERE id = $8 AND deleted_at IS NULL
RETURNING id, parent_ref, service, port, leader, drain_backends, domain, tls_mode, managed_certificate_domain, created_at, updated_at`
	tcpRouteDeleteQuery = `
...
VALUES ($1, $2)`
	tcpRouteCertificateDeleteByRouteIDQuery = `
DELETE FROM tcp_route_certificates
WHERE tcp_route_id = $1`
	tcpRouteCertificateInsertQuery = `
INSERT INTO tcp_route_certificates (tcp_route_id, certificate_id)
VALUES ($1, $2)`
