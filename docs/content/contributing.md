...
This fork does **not** use the Developer Certificate of Origin. Use plain `git commit` with a Conventional Commit subject; do not add `Signed-off-by` trailers (`git commit -s`).

## Pull request procedure

1. Branch from `main` (not a long-lived personal copy of `master`).
1. Rebase onto current `main`.
1. Run the tests you can (`make test-unit`; `script/vagrant-smoke.sh --item quick` if the change needs a cluster; `singleton`/`ha` before a release).
...
Target the `main` branch of [randy-girard/flynn](https://github.com/randy-girard/flynn).
...
Conduct notes live in [`CONTRIBUTING.md`](../../CONTRIBUTING.md) at the repository root.
