...
| `db-runtime` / `db-runtime:create` / `db-runtime:update` / `db-runtime:remove` / `db-runtime:allow-custom` | CPU, memory, and disk presets per database engine (postgres, redis, mariadb, mongodb, kafka, clickhouse). Not app process runtimes. Builtins are in memory; edits are stored in `/etc/flynn/db-runtimes.json` (`FLYNN_DB_RUNTIMES`). `resource:add --runtime` uses this catalog (default `small`). Updating a definition does not resize existing instances. |
